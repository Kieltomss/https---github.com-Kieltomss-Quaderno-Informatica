...
<body>
    ...
    <pre>
    1. <b>Form di Login</b>: Il modulo HTML permette all'utente di inserire un nome utente e una password. 
       I dati vengono inviati al server tramite il metodo `POST` alla stessa pagina (`EsercizioA.php`).

    2. <b>Controllo delle credenziali</b>:
       - Dopo che l'utente invia i dati, PHP verifica se il nome utente e la password corrispondono ai valori predefiniti: 
         `admin` come nome utente e `password` come password.
       - Se le credenziali non corrispondono, viene mostrato un messaggio di errore.
       - Se le credenziali sono corrette, viene mostrato un messaggio di benvenuto personalizzato con il nome dell'utente.

    3. <b>Link per tornare indietro</b>: Un semplice link permette di tornare alla pagina precedente.
    </pre>
